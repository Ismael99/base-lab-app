...
export const examenRealizado = (Yup) => ({
  validations: Yup.object({
    examen_realizado_resultados: Yup.array(
      Yup.object().shape({
        label: Yup.string().min(3, "Carácteres mínimos 3").required("Ingrese nombre del campo"),
        value: Yup.string().min(3, "Carácteres mínimos 3").required("Ingrese resultado de examen"),
        rn: Yup.string(),
      })
    ...
    examen_realizado_quimico: Yup.array(
      Yup.object().shape({
        ...
        label: Yup.string().min(2, "Caráteres mínimos 2").required("Campo requerido"),
      })
    ).required("Campo requerido"),
  }),
  initialValues: {
